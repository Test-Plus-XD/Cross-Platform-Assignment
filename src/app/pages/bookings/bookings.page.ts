// Bookings page for customers to view and manage their reservations
...
export class BookingsPage implements OnInit, OnDestroy {
  ...
  translations = {
    ...
    all: { EN: 'All', TC: '全部' },
    noBookings: { EN: 'No bookings found', TC: '沒有預約記錄' },
  ...
  loadBookings(forceRefresh: boolean = false): void {
    ...
    this.bookingService.getMyBookings(forceRefresh)
  ...
  getFilterCount(filter: BookingFilter): number {
    ...
    switch (filter) {
      case 'upcoming':
        return this.bookings.filter(b => new Date(b.dateTime) >= now && b.status !== 'cancelled').length;
      case 'past':
        return this.bookings.filter(b => new Date(b.dateTime) < now && b.status !== 'cancelled').length;
      case 'cancelled':
