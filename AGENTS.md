...
3. This AGENTS.md file (if major changes)
...
5. Update this AGENTS.md if major changes
...
**Files modified:** `src/app/app.component.html`, `src/app/pages/store/store.page.html`, `src/app/app.module.ts`, `src/style/global.scss`, `AGENTS.md`.
...
10. **Never use `<ion-router-outlet>` in a custom div shell** — when `<ion-router-outlet>` is not a direct child of `<ion-app>`, its internal `containerEl.commit()` Promise hangs on Android WebView. Use standard Angular `<router-outlet>` instead. **The `router-outlet + *` sizing CSS MUST live in `global.scss`**, not in a component stylesheet — Angular's `ViewEncapsulation.Emulated` scopes component styles with `[_ngcontent-XXX]` attributes, but dynamically inserted route components don't carry these attributes, so the selector never matches. This was the failure mode of v1.17.10 (wrong element) and the initial v1.17.11 attempt (right selector, wrong file).
