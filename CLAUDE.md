...
> **Last Updated:** 2026-04-15 | **Version:** 1.17.25 | **Angular:** 20.3.3 | **Ionic:** 8.7.9
...
**Document Version:** 1.17.27 | **Maintainer:** AI Assistant

**Changelog:**
- **v1.17.27** (2026-04-16): **Login/register password visibility toggle, QR scanner dark-mode transparency/size tweak, native Stripe custom tab flow, and hardened account-type setup gating.** `src/app/pages/login/login.page.html/ts` now adds a password visibility toggle with `eye-off-outline` (default hidden) and `eye-outline` (shown) for both login and register modes. QR scanner UI was adjusted with a mildly larger viewfinder square and stronger transparent-native modal rules so dark mode no longer paints over the native camera feed (`src/app/shared/qr-scanner/qr-scanner-modal.component.scss`, `src/style/global.scss`). Stripe checkout now opens via Capacitor Browser on native builds (Chrome Custom Tabs / Safari view controller) while web keeps full-page redirect; success/cancel URLs now use `pourrice://store...` deep links on native so the app can resume Store flow (`src/app/pages/store/store.page.ts`, `src/app/app.component.ts`). The user account-type setup flow was hardened to be truly blocking and foregrounded: full-viewport modal style, full-height sheet presentation, enforced routing to `/user` until setup completion, and race-safe modal presentation driven by combined auth/profile streams (`src/app/app.component.ts`, `src/style/global.scss`). Verified with `npm run -s build:tailwind` and `npx ng build`.
