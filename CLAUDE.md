...
**Vercel-specific follow-up (v1.17.32):** Runtime logs later showed token registration succeeding but no subsequent `[FCM] Chat ...` send logs after `POST /API/Chat/Rooms/:roomId/Messages`, which indicated the push send was not surviving the request lifecycle. The actual remaining issue was that `Chats.ts` and `Bookings.ts` triggered `sendChatMessageNotification()` / `sendBookingStatusNotification()` as fire-and-forget promises (`.catch(...)` without `await`). On Vercel's serverless runtime that is unsafe, because once the HTTP response is returned the invocation may stop before the background FCM work completes. The notification calls are now awaited inside local `try/catch` blocks so the request stays alive until the push attempt and its logs finish, whilst still preserving the saved message / booking update even if FCM itself fails.
...
- **v1.17.41** (2026-05-01): **Store restaurant add/edit modal unified with Ionic time pickers.** `src/app/pages/store/add-restaurant-modal/*` now serves both create and edit flows through a `mode: 'create' | 'edit'` input, preloading existing restaurant data in edit mode whilst retaining image upload support. `src/app/pages/store/store.page.ts/html` now opens this shared modal for both "Add New Restaurant" and "Edit Info", passing the current `restaurantId` and restaurant object for edits. `StorePageModule` no longer declares the old `RestaurantInfoModalComponent`, and `src/app/pages/store/restaurant-info-modal/*` was removed. Opening hours no longer rely on manual text input: each weekday now has open/close buttons backed by an Ionic `ion-datetime` time picker (`presentation="time"`, 24-hour `en-GB` locale), plus a closed toggle and clear action, while still storing the existing `"HH:MM-HH:MM"` / `"Closed"` map shape for API compatibility. Verified with `npx ng build`; browser/dev-server smoke testing was skipped at user request. Existing CommonJS warnings for `qrcode`/`jsqr` and the Node 25.2.1 non-LTS warning remain unchanged.
...
- **v1.17.37** (2026-04-29): **Account-type setup modal visibility and flow fix.** `src/style/global.scss` now gives `ion-modal.account-type-modal` an explicit responsive height instead of `--height: auto`, which could collapse the Ionic modal wrapper on Android/WebView and leave only the backdrop visible. `src/app/shared/account-type-selector/account-type-selector.component.*` now uses a two-step setup flow: choosing Diner/Restaurant immediately advances to a basic account setup screen, and the backend save only happens on the final Finish action. The save path now uses a 15s timeout plus a create-profile fallback when a newly authenticated user hits the profile creation race, so the UI shows an error instead of spinning forever. Follow-up fix in `src/app/app.component.ts`: `canDismiss` is now a role-based guard that blocks backdrop/hardware escape dismissals but allows the successful `confirm` dismissal, preventing the modal from staying stuck on "Saving..." after the profile update succeeds. Verified with `npx ng build` and `npx cap copy android`.
- **v1.17.36** (2026-04-24): **Restaurant detail menu and map polish.** `src/app/pages/restaurant/restaurant.page.html/scss/ts` now keeps Opening Hours permanently expanded, wraps the Address/Map area and its Directions action in the same card colour system as the other restaurant segments, and restyles Directions plus View Full Menu with the booking-button gradient treatment. The Menu preview now samples five random menu items after loading and displays them inside a fixed-height scroller sized for three visible rows. Added `src/app/pages/restaurant/full-menu-modal.component.ts` for the full bilingual menu list and wired `pourrice://menu/{restaurantId}` through `src/app/app.component.ts` and `src/app/services/messaging.service.ts` to `/restaurant/{restaurantId}?menu=open`, so QR/notification deep links open the modal after menu data settles. Follow-up in the same version aligns Contact card icons to the universal `--gradient-icon-color`, forces Reserve/Directions/View Full Menu icons to inherit their button text colour, keeps the map-link icon on the gradient token, and moves the distance badge to the right side of the map metadata row using the login/register green tokens. Updated the messaging spec expectation for the new menu route. Verified with `npx ng build`.
- **Session note** (2026-04-24): Installed the local Codex skill bundle `C:\Users\Test-Plus\Downloads\content-ppt.skill` into `C:\Users\Test-Plus\.codex\skills\science-content-ppt`. This adds the `science-content-ppt` skill and its bundled HTML presentation templates for future Codex sessions. No Ionic project source code was changed.
- **v1.17.35** (2026-04-24): **Chat page cache now updates live when in-app messages arrive.** `src/app/pages/chat/chat.page.ts` now subscribes to `ChatService.Messages$` so Ionic’s cached Chat page mutates its in-memory `chatRooms` list as soon as a real-time socket message arrives, instead of waiting for a manual reload. The room preview cache updates `lastMessage`, `lastMessageAt`, `messageCount`, and room ordering, and will seed a missing room into the list if the cached page had not seen it yet. The Chat page also gained `ionViewWillEnter()` reconciliation so a reused Ionic route quietly refreshes chat records from `/API/Chat/Records/:userId` without discarding the existing cache on transient refresh failures. Verified with `npx ng build`. A broader `npx tsc -p tsconfig.json --noEmit` run still reports unrelated pre-existing repo issues in spec typings and constant exports, so Angular build output remains the meaningful validation path for this change.
- **v1.17.34** (2026-04-23): **Android-native chat and booking pushes now bypass collapsible generic notification delivery.** User testing on `com.example.cross_platform_assignment` showed a precise pattern: the first incoming chat notification appeared, but later messages from the second account were often dropped even though the Vercel API logged successful sends. The sibling API route `..\Vercel-Express-API\API\Routes\Messaging.ts` now splits fan-out by platform and sends chat/booking updates to tagged `android-native` tokens as high-priority data-only FCM payloads carrying `title`, `body`, and a stable `notificationTag`, while web/iOS and legacy tokens keep the standard notification payload. On the Android app side, `android/app/src/main/java/com/example/cross_platform_assignment/PourRiceMessagingService.java` now replaces the plugin manifest service, forwards tokens and message events back into `@capacitor-firebase/messaging`, creates the `pourrice_default_notifications` channel on cold starts, and renders unique local notifications for background or terminated Android-native pushes. `android/app/src/main/AndroidManifest.xml` removes the plugin's default `MessagingService` entry and registers `PourRiceMessagingService`; `src/app/services/messaging.service.ts` now falls back to `data.title` / `data.body` when data-only Android notifications reach the foreground listener. Because `PourRiceMessagingService.java` is app-module Java code that directly imports `com.google.firebase.messaging.FirebaseMessagingService` and `RemoteMessage`, the Android app module must also declare a direct Firebase Messaging compile dependency: `android/gradle/libs.versions.toml` defines `firebase-messaging = "25.0.1"` / `libs.firebase.messaging`, and `android/app/build.gradle` includes `implementation libs.firebase.messaging`. The Node package `@capacitor-firebase/messaging` is still used for the Capacitor bridge and token/listener APIs; the Gradle dependency is the native Android SDK dependency needed by the custom service. This specifically targets the “first message arrives, later ones vanish” Android behaviour.
- **v1.17.33** (2026-04-21): **Stripe ad-placement 2h grace window now survives modal cancellations and app quits.** The existing `pendingAdSession` localStorage entry was correctly written and had a 2h validity check inside `checkPendingAdSession()`, but three unconditional `localStorage.removeItem(PENDING_AD_SESSION_KEY)` calls wiped it out before the window could be used: one in `checkPendingAdSession()` right before opening the modal, one when the same session had already been handled in the current component lifetime, and one in `openAdModal()` that ran on every modal dismissal regardless of outcome. `src/app/pages/store/store.page.ts` now keeps the localStorage entry intact until the ad is actually created (`data?.created === true`) or the 2h window elapses. A reactive `pendingAdSession` mirror of the stored entry is refreshed on init, on `ionViewWillEnter`, after Stripe return, and once per minute via a timer, so the ads tab can render a warning-coloured resume banner with a live "time remaining" countdown and a "Complete Advertisement" button wired to `resumePendingAdPlacement()`. `checkPendingAdSession()` was rewritten on top of the new `refreshPendingAdSession()` helper and now only guards against re-popping the modal (via `lastHandledStripeSessionId`) without discarding the entry. `src/app/pages/store/store.page.html` adds the banner above the existing "Place New Advertisement" CTA in the ads section, and `src/app/pages/store/store.page.scss` adds only a scoped `.pending-ad-card` block for title/timer row alignment (no `transition: all`). Verified with `npx ng build --configuration development`.
- **v1.17.32** (2026-04-20): **Vercel serverless FCM send path fixed by awaiting notification work.** Deployed logs showed successful `POST /API/Messaging/register-token` calls for `android-native, com.example.cross_platform_assignment` but no subsequent `[FCM] Chat ...` send logs after `POST /API/Chat/Rooms/:roomId/Messages`, even though message persistence succeeded. The sibling API routes `..\Vercel-Express-API\API\Routes\Chats.ts` and `..\Vercel-Express-API\API\Routes\Bookings.ts` had been launching `sendChatMessageNotification()` / `sendBookingStatusNotification()` as fire-and-forget promises, which is unreliable on Vercel serverless once the HTTP response is returned. Both routes now `await` the notification attempt inside `try/catch` blocks so the runtime stays alive long enough for FCM sends and logs to complete. `..\Vercel-Express-API\API\Routes\Messaging.ts` also now logs when a chat recipient has no tagged FCM tokens, which removes the previous silent skip during chat fan-out.
...
- **v1.17.31** (2026-05-02): **Account setup modal OAuth name prefill + action button colour fix.** `src/app/shared/account-type-selector/account-type-selector.component.ts` now resolves the setup display-name default from the freshest available sources (`currentProfile.displayName` → `appState.displayName` → email prefix) so OAuth registrations pre-populate the Display Name field even when profile/auth timing differs. The step-2 Back/Finish actions in `account-type-selector.component.html` now use filled semantic colours (`warning` for Back, `success` for Finish), and `account-type-selector.component.scss` sets matching contrast text colours for both buttons.
...
- **v1.17.46** (2026-05-05): **Map view Load More button for Algolia pagination.** `src/app/pages/search/search.page.ts` adds `isMapLoadingMore` flag, a `mapRemainingCount` getter (`totalResults - restaurants.length`), and `loadMoreMapResults()` which increments `currentPage` and calls `performSearch(true)` (append mode) without destroying the map or triggering the full loading state. `src/app/pages/search/search.page.html` adds a floating pill `<button class="map-load-more-btn">` inside `.map-view-container`, visible only when `!isNearMeActive && currentPage < totalPages - 1`; it shows an Eclipse.gif mini-spinner while loading and is disabled during the fetch. `src/app/pages/search/search.page.scss` adds `position: relative` to `.map-view-container` and styles the button as an `position: absolute` bottom-centre overlay with primary-colour background, hover lift, and disabled-state opacity. Verified with `npx ng build --configuration development`; only pre-existing CommonJS warnings remain.

- **v1.17.45** (2026-05-05): **Google Maps Places Autocomplete + reverse geocoding in restaurant add/edit modal.** `src/app/pages/store/add-restaurant-modal/add-restaurant-modal.component.ts` now initialises a `google.maps.places.Autocomplete` (HK-restricted, `fields: ['geometry']`) on a plain `<input>` above the map, and a `google.maps.Geocoder` instance at the end of `initializeMap()`. A new `reverseGeocode()` async method fires parallel EN + `zh-TW` geocoder calls on every pin placement (both Autocomplete selection and map click), strips the trailing `, Hong Kong` / `, 香港` country suffix from `formatted_address`, and auto-fills `form.Address_EN`, `form.Address_TC`. District is extracted from `administrative_area_level_2` in the EN result, the `Central and Western` → `Central/Western` normalisation applied, and matched against the 18-district constants to set `form.District_EN` / `form.District_TC`. `onMapClick()` now delegates to `placePin()` (shared with the Autocomplete handler) then calls `reverseGeocode()`. `destroyMap()` calls `google.maps.event.clearInstanceListeners` on the autocomplete instance to prevent listener leaks. A public `isGeocoding` flag drives a crescent spinner inside the search bar and `[disabled]` bindings on the Address EN/TC inputs and District picker row. `src/app/pages/store/add-restaurant-modal/add-restaurant-modal.component.html` adds the `.location-search-bar` block (plain `<input id="restaurant-location-search">`, search icon, geocoding spinner) above the map canvas. `src/app/pages/store/add-restaurant-modal/add-restaurant-modal.component.scss` adds `.location-search-bar` styles inside `.map-container` using scoped transitions (`opacity` only — no `transition: all`). `src/style/global.scss` adds a `.pac-container { z-index: 99999 !important }` block with light/dark theme styling so the Places autocomplete dropdown renders above Ionic modal backdrops. Verified with `npx ng build`; only pre-existing CommonJS warnings for `qrcode`/`jsqr` remain.
...
- **v1.17.50** (2026-05-06): **Menu item image upload path alignment.** `DataService.uploadFile()` now accepts an optional `POST`/`PUT` method while preserving `POST` as the default for existing image callers. `RestaurantsService.uploadMenuItemImage()` now calls `PUT /API/Restaurants/:restaurantId/menu/:menuItemId/image`, lets the API write `imageUrl`, and invalidates the restaurant menu cache instead of uploading through the restaurant hero endpoint and doing a second `updateMenuItem()` write. Bulk DocuPipe import parsing now prefers `imageUrl` and keeps `image` as a compatibility fallback. Sibling API changes align storage to `Menus/{restaurantId}/` and menu document field writes to `imageUrl`; the Flutter Android variant now reads/writes `imageUrl` while preserving legacy `image` response compatibility. Verified `npx ng build --configuration development`; only the existing Node 25 non-LTS and CommonJS warnings remain. API typecheck could not complete because the API repo dependencies are not installed and `npm install` was not approved. Android targeted `dart analyze lib\models\menu.dart lib\services\menu_service.dart lib\widgets\menu\menu_item_form.dart lib\pages\store_menu_manage_page.dart` passes.

- **v1.17.51** (2026-05-06): **Store restaurant/menu modal UI, themed time picker, and payment display consistency.** `src/app/pages/store/add-restaurant-modal/add-restaurant-modal.component.html/scss` now uses an icon-only header close action, a real flex-column fullscreen modal layout, normal 16px content padding, no artificial `.bottom-spacer`, and a sticky two-button footer matching the add/edit menu item modal (`Save Changes` plus outlined `Cancel`). `src/app/pages/store/menu-item-modal/menu-item-modal.component.html/scss` now uses the same flex layout, 16px Store-page horizontal content rhythm, themed background, card-like fields/image preview, and matching stacked footer button styling as the restaurant form modal. Both modal footers now use the same primary-to-secondary gradient surface as their headers. The opening-hours picker now uses an Ionic `ion-popover` with native backdrop/dismissal instead of the custom fixed overlay, and `src/style/global.scss` adds theme RGB tokens plus scoped `.restaurant-time-picker-popover` rules so the `ion-datetime` wheel, highlight, text, and surface follow the app's light/dark green palette without the previous white dark-mode panel or right-edge gap. Payment methods in the restaurant add/edit modal are normalised to English before storing, including legacy TC values loaded for editing. `src/app/pages/restaurant/restaurant.page.html/ts` now moves Payment Methods after Number of Seats and before Opening Hours, and renders payment labels through `PaymentMethods` so EN/TC users see translated labels while storage remains English. The existing edit-mode update flow remains unchanged: `save()` still calls `saveExistingRestaurant()`, then dismisses with `{ saved: true }` so Store reloads the restaurant after updates. Verified with `npx ng build`; only the existing Node 25 non-LTS and CommonJS warnings for `qrcode`/`jsqr` remain.
